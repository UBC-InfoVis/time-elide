<script>
  ...
  $: data.forEach(slice => {
    slice.xScaleCustom = d3.scaleTime()
    ...
    {#each data as slice, index}
      <g transform="translate({xScale(slice.xPos)},0)">
        ...
        <rect
          class="ts-iqr" 
          width={xScale(slice.duration)}
          height={yScale(slice.lowerQuartileValue) - yScale(slice.upperQuartileValue)}
        ...
        <text class="ts-x-label" y={height + 20} x={xScale(slice.duration)/2}>{index + 1}</text>
      </g>
    {/each}
